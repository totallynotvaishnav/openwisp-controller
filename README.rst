...
Like `Default templates <default-templates>`__ but cannot be removed from a device
configuration, they can only be overridden.

Will be always assigned earlier than default templates,
so they can be overridden if needed.

In the example above, the "SSID" template is flagged as "(required)" and its checkbox
